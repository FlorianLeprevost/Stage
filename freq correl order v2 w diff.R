...
means_diff = c(mean(dtf$interval.diff[1:vlines[1]], na.rm=T), mean(dtf$interval.diff[vlines[1]:vlines[2]]), mean(dtf$interval.diff[vlines[2]:vlines[3]]), mean(dtf$interval.diff[vlines[3]:vlines[4]]), mean(dtf$interval.diff[vlines[4]:vlines[5]], na.rm=T))
means_diff = round(means_diff, 5) *100

x=data.frame(vlines,means_low)
y=data.frame(vlines,means_diff)
...
dtf %>%
  ...
  geom_point(data=x, aes(x=vlines -100, y=means_low, color="red", size=4), show.legend= FALSE)+
  annotate("text", x = vlines -100,y = (means_low + 1)^2 -.98, label = means_low)+
  geom_point(data=y, aes(x=vlines -100, y=means_diff, color="green", size=4), show.legend= FALSE)+
  annotate("text", x = vlines -100,y = (means_low + 1)^2-.95, label = means_diff)+
  ylab("power")+
  ggtitle("mean of power of HRV LF in bins sorted by order")


ggsave("2757mean_lowrrr.jpg")
