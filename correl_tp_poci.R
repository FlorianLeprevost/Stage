...
x=cor(dtf_coeur[,1:800], dtf_poci23[,1:800], use="complete.obs")
...
plot_ly(z = x, type = "heatmap", zmin=-.05, zmax=.05 )%>%
  layout(xaxis = list(title="poci12 time point"), 
...
signif_vec = as.numeric(signif_vec)/400
index = 1:length(correl_signif)
mean_elec = colMeans(dtf_poci23)/150
mean_elec = mean_elec[1:800]

dtf_plot = data.frame(index, correl_vec, signif_vec, mean_elec)

ggplot(dtf_plot)+
  geom_line(aes(x=index, y= correl_vec), color="blue", size=1)+
  geom_line(aes(x=index, y= mean_elec), color="grey",size=.5)+
  geom_area(aes(x=index, y= signif_vec))+
  ylab("correlation between ECG and poci23 trials")+
  ggtitle("Correlation ECG & poci23")
ggsave("poci23 diag.jpg")
# # 
# 
# dtf_plot = data.frame(index, correl_vec, signif_vec, mean_elec)
# 
